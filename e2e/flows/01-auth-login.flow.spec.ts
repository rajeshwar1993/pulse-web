...
test.describe("01 — Auth Login Flow @smoke", () => {
  ...
  test("User A logs in successfully and reaches dashboard", async ({
    browser,
  }) => {
  ...
  test("login page has signup and forgot-password links", async ({
    browser,
  }) => {
    const context = await browser.newContext({
      storageState: { cookies: [], origins: [] },
  ...
  test("already-authenticated user visiting /auth/login redirects to /dashboard", async ({
    page,
  }) => {
