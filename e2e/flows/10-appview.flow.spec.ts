...
test.describe('10 — AppView Flow', () => {
  ...
  test('dashboard sends FlutterBridge ready signal', async ({ page, flutterBridge }) => {
  ...
  test('locale change via bridge → localStorage updated', async ({ page, flutterBridge }) => {
    await page.goto('/appview/dashboard');
    await expect(page.locator('h1').first()).toBeVisible();
    ...
    const locale = await page.evaluate(() => localStorage.getItem('pulse-locale'));
    expect(locale).toBe('en');
  });

  test('navigation: dashboard → settings → back', async ({ page, flutterBridge }) => {
  ...
  test('settings shows profile card + edit link', async ({ page, flutterBridge }) => {
    const settings = new SettingsPage(page);
    await settings.goto();
