...
async function setupUser(user: { email: string; password: string; displayName: string }) {
  const userId = await ensureAuthUser(user.email, user.password, user.displayName);
